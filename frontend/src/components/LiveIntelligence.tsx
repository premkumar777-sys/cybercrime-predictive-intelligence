...
export function LiveInvestigator({ caseId, go }: { caseId: string | null; go: Go }) {
  ...
  <div className="flex items-center justify-between border-b border-border pb-5">
    <div><p className="text-xs uppercase text-muted-foreground">Cash-withdrawal likelihood</p><p className="mt-2 text-4xl font-extrabold text-destructive">{prediction?.status === "INSUFFICIENT_DATA" ? "N/A" : scorePercent(topScore)}</p></div>
    <div className="text-right">
      ...
      {prediction && <p className="text-[10px] text-muted-foreground">Data as of: {now.toLocaleTimeString()}</p>}
    </div>
  </div>
  {prediction?.status === "INSUFFICIENT_DATA" ? <p className="mt-4 border-l-4 border-destructive bg-destructive/10 p-3 text-xs font-bold text-destructive">{prediction.reason}</p> : <p className="mt-4 border-l-4 border-accent bg-muted p-3 text-xs leading-relaxed text-muted-foreground">Probabilistic estimate only. Use with corroborating evidence and field verification.</p>}
  <Button className="mt-5" onClick={analyze} disabled={busy}>{busy ? "Analysing..." : prediction ? "Re-run analysis" : "Analyse case"}<RefreshCw /></Button>{error && <p className="mt-4 text-xs text-destructive">{error}</p>}</Panel>
